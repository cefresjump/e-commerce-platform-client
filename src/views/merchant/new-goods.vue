...
const figureURL = computed(() => {
  if (goodsData.value.figure != '') {
    return `http://localhost:30000/static/${goodsData.value.figure}.webp`;
  } else return '';
})
...
const submitForm = async () => {
  const data = {
    merchantId: merchantAuthStore().merchantId,
    ...
    figure: goodsData.value.figure
  }
  const result = await goodsApi.addInfo(data.merchantId, data.name, data.type, data.price, data.shippingAddress, data.description, data.figure);
  if(result){
    ElMessage({
      message: '新增商品成功',
      type: 'success'
...
<template>
  <div class="container">
    <div class="header">
      ...
      <a style="padding-left: 20px;">进货信息录入</a>
    </div>
    <el-form label-width="80px" :model="goodsData" class="goods-form">
      ...
      <el-form-item label="商品图片" prop="goodsData.figure">
        <el-upload class="avatar-uploader" name="figure" action="http://localhost:30000/upload" :show-file-list="false"
          ...
          <img v-if="figureURL" :src="figureURL" class="figure" />
