...
const login = async () => {
  ...
  if (accountMsg.name === '' && accountMsg.password === '') {
    ...
    const result = await merchantApi.login(accountMsg.name, accountMsg.password)
    if (result) {
      merchantAuthStore().setUser(result.id, accountMsg.name)
      submitMessage.value = '登录成功，正在跳转。。。'
      setTimeout(() => router.push('/merchant/info'), 1000)
    } else {
...
const jumpMerchantReg = () => {
  router.push('/account/merchantRegister')
}
</script>
...
<template>
  <div class="container">
    ...
    <div class="content">
      <img src="../../assets/img/merchant-login.webp" style="max-height: 500px" />
      <el-form :model="loginForm" status-icon ref="loginFormRef" class="login-form">
        ...
        <div class="jump-register" @click="jumpMerchantReg">立即注册</div>
      </el-form>
    </div>
  </div>
</template>
