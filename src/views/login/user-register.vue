...
async function submitForm() {
  ...
  if (!(accountMsg.name.length <= 20)) {
    ...
    const result = await userApi.register(accountMsg.name, accountMsg.password, accountMsg.email)
    if (result) {
      submitMessage.value = '注册成功，正在跳转。。。'
      setTimeout(() => router.push('/account/userLogin'), 1000)
    } else {
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="toMerchantReg">
        <div @click="jumpMerchantReg">前往商户注册</div>
      </div>
    </div>
  </div>
</template>
